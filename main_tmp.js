...
// ===== SOFFIT GLOBALS =====
let sMaterial, sOverhang, sLength, sTrimP, sTrimJ, sTrimFacia, sTrimCorner;
let sCalcBtn, sPdfBtn, sRes, sErr;
let sLast = null;

document.addEventListener('DOMContentLoaded', () => {
 console.log('EST calculators build v31 (2026-03-02)');
 // ============================================================
...
function sizeBySpan(span){
 ...
 function jCalc(){
  ...
  data.forEach(s => {
   ...
   const workM = (s.name === 'fermer') ? 0.145 : 0.110; // рабочая ширина ламели (м)
   const lamelQty = Math.ceil(s.height / workM) * s.sectionsQty;
...
function roundToCmM(m){
 return Math.round(m * 100) / 100; // 0,01 м
}
function aFmtM(v){
 const x = Number(v);
...
function buildPickersExcelSmooth(span, edgeH, centerH, plankW, gap, offset){
 ...
 // 3) -0,01 м всем кроме первой/последней
 for (let i = 1; i < N - 1; i++){
  smooth[i] -= 0.01;
 }
 ...
 const centerRounded = roundToCmM(centerH);
 const edgeRounded = roundToCmM(edgeH);
...
function renderArchedTable(rowA, rowB){
 ...
 let html = `
  <table class="small-table">
   <thead>
    <tr>
     <th>№</th>
     <th>Высота (ряд 1), м</th>
     <th>Высота (ряд 2), м</th>
    </tr>
   </thead>
   <tbody>
 `;
 ...
 for (let i = 0; i < maxN; i++){
  html += `
   <tr>
    <td>${i + 1}</td>
    <td>${rowA[i] != null ? aFmtM(rowA[i]) : '—'}</td>
    <td>${rowB[i] != null ? aFmtM(rowB[i]) : '—'}</td>
   </tr>
  `;
 }
...
function aCalc(){
 ...
 const gap = Number(aGap?.value) / 100; // cm -> m
 const chess = aChess?.value === 'yes';

 if (!isFinite(span) || span <= 0) { if(aErrEl) aErrEl.textContent = 'Введите ширину пролёта (м) > 0'; return; }
 if (!isFinite(edgeH) || edgeH <= 0) { if(aErrEl) aErrEl.textContent = 'Введите высоту у стоек (м) > 0'; return; }
 if (!isFinite(centerH) || centerH <= 0) { if(aErrEl) aErrEl.textContent = 'Введите высоту по центру (м) > 0'; return; }
 ...
 if (aMinEl) aMinEl.textContent = aFmtM(minH);
 if (aMaxEl) aMaxEl.textContent = aFmtM(maxH);
...
function downloadArchedPdf(){
 ...
 const info = [
  ['Ширина пролёта, м', aFmtM(span)],
  ['Высота у стоек, м', aFmtM(edgeH)],
  ['Высота по центру, м', aFmtM(centerH)],
  ['Ширина планки, м', aFmtM(plankW)],
  ['Зазор, м', aFmtM(gap)],
 ...
 if (doc.autoTable) {
  ...
  const sumBody = aLastSummaryRows.map(r => [aFmtM(r.h), String(r.qty)]);
  doc.autoTable({
   head: [['Высота H(x), м', 'Кол-во, шт']],
   body: sumBody,
   startY: yAfter,
   theme: 'grid',
   styles: {
  ...
  doc.autoTable({
   head: [['№', 'Высота (ряд 1), м', 'Высота (ряд 2), м']],
   body: rowsBody,
   startY: yAfter,
   theme: 'grid',
   styles: {
...
if (fWallsWrap && fAddWallBtn && fCalcBtn && fPdfBtn && fTableWrap && fErr) {
 ...
 function renderResultTable(res){
  ...
  const rowProfiles = ['Количество профилей (Г/П), 3 м (шт)'];
 ...
 function downloadFacadeSubsysPdf(){
  ...
  const calcBody = [
   ['Количество профилей (Г/П), 3 м (шт)', ...lastWallsResult.profiles.map(x => String(x))],
  ...
  const specBody = [
   ['Профиль (Г/П), 3 м', 'шт', String(totalProfiles)],
...
const gValleys = g('g_valleys');
const gValleysWrap = g('g_valleys_wrap');
...
function gRenderType(){
 ...
 gUpdateValleysVisibility();
}

function gNeedsValleys(){
 const isMetal = (gType?.value === 'metal');
 if (!isMetal) return false;
 const b = String(gMetalBrand?.value || '');
 return (b.startsWith('vegaprom') || b === 'docke_premium_stal' || b === 'gl_granite_125x90' || b === 'gl_granite_150x100');
}

function gUpdateValleysVisibility(){
 if (!gValleysWrap) return;
 const isOther = (gRoofTypeInput?.value === 'other');
 const show = isOther && gNeedsValleys();
 gValleysWrap.classList.toggle('hidden', !show);
 if (!show && gValleys) gValleys.value = 0;
}
...
function gCalc(){
 ...
 const L = Number(String(gLength.value || '').replace(',', '.'));
 const H = Number(String(gHeight.value || '').replace(',', '.'));
...
const { ext, intr } = gGetAnglesAndEnds();
const roofMountedYes = (gRoofMounted?.value === 'yes');

const isMetal = gType.value === 'metal';
const metalBrand = gMetalBrand?.value || 'vegaprom_185x140_zn';
const plasticBrand = gPlasticBrand?.value || 'docke_lux';

const roofType = gRoofTypeInput?.value || 'single';
const overhang = Number(String(gOverhang?.value || '').replace(',', '.'));
const valleys = (roofType === 'other' && gNeedsValleys())
 ? Number(String(gValleys?.value || '').replace(',', '.'))
 : 0;

// 1) Желоба: Длина по карнизу / длина желоба + 1
const gutterLen = (isMetal && String(metalBrand).startsWith('vegaprom')) ? 2 : 3;
const gutters = gCeil(L / gutterLen) + 1;

// 2) Держатели желоба / крюк / кронштейн: Длина по карнизу / 0,6
const holders = gCeil(L / 0.6);

// 3) Заглушки: только односкатная/двускатная/мансардная
let caps = 0;
if (roofType === 'single') caps = 2;
if (roofType === 'double') caps = 4;
if (roofType === 'mansard') caps = 4;

// 4) Углы: только вальмовая/шатровая (4 шт) или по введённым углам
let corners = 0;
if (roofType === 'hip' || roofType === 'tent') corners = 4;
else corners = (Number(ext) || 0) + (Number(intr) || 0);

// 5) Воронки выпускные: Длина по карнизу / 8
const funnelsOut = gCeil(L / 8);

// 6) Воронки водосборные: = кол-ву ендов
const funnelsWater = (isFinite(valleys) && valleys > 0) ? Math.round(valleys) : 0;

const funnelsTotal = funnelsOut + funnelsWater;

// 7) Колена трубы
const elbows = (overhang > 0) ? (funnelsTotal * 2) : funnelsTotal;

// 8-9) Трубы: вертикаль 3 м + добор остатка 1 м/2 м (где есть 2 м) + добор на свес
const mb = String(metalBrand);
const hasPipe2 = (isMetal && (
 mb.startsWith('gl_optima') ||
 (mb.startsWith('vegaprom') && !mb.startsWith('vegaprom_185x140_pe') && mb !== 'vegaprom_200x180_zn' && mb !== 'vegaprom_200x180_pe')
));
...
const full3 = Math.floor(H / 3);
const rem = H - full3 * 3;

pipes3 = full3 * funnelsTotal;

if (rem > 0.000001){
 if (rem <= 1){
  pipes1 += funnelsTotal;
 } else if (rem <= 2){
  if (hasPipe2) pipes2 += funnelsTotal;
  else pipes1 += 2 * funnelsTotal;
 }
}

if (overhang > 0) pipes1 += funnelsTotal;


// 10) Соединители желобов
const gutterConnectors = Math.max(0, gutters - 1);

// 11) Кронштейны трубы / хомуты трубы / держатели трубы
const pipeHolders = pipes3 * 2;

// 12) Наконечники / Колено слива / Отмет
const tips = funnelsTotal;

// Муфта соединительная: по вашей логике ("н" = 0)
const couplings = Math.max(0, (pipes1 + pipes2 + pipes3) - 1);
...
if (isMetal){
 // ===== METAL =====
 if (String(metalBrand).startsWith('vegaprom')){
  const vb = String(metalBrand);
  const hasPipe2000 = !(vb.startsWith('vegaprom_185x140_pe') || (vb === 'vegaprom_200x180_zn') || (vb === 'vegaprom_200x180_pe'));
  const hasPipe3000 = !((vb === 'vegaprom_200x180_zn') || (vb === 'vegaprom_200x180_pe'));

  push('Желоб 2 м', gutters);
  push('Крепление желоба вертикальное', holders);
  push('Усилитель крепления желоба', holders);

  push('Заглушка желоба', caps);
  push('Желоб угловой 90°', corners);

  // для пластика — одна позиция "Воронка"
  push('Воронка', funnelsOut);

  push('Колено', elbows);

  // труба 3 м (если доступна)
if (hasPipe3000){
 const vpPipes3 = Math.floor(H / 3) * funnelsTotal;
 push('Труба 3 м', vpPipes3);
}

// трубы 1250/2000
if (vb === 'vegaprom_200x180_zn' || vb === 'vegaprom_200x180_pe'){
 // Vegaprom 200/180: Труба 1250 = ceil(H/1.25) + кол-во воронок
 const vp1250 = (Math.ceil(H / 1.25) + 1) * funnelsTotal;
 push('Труба 1,25 м', vp1250);
} else {
 // остальные VEGAPROM: трубы по общей логике остатка высоты (1250 ~ 1 м, 2000 ~ 2 м) + добор на свес
 push('Труба 1,25 м', pipes1);
 if (hasPipe2000) push('Труба 2 м', pipes2);
}


  push('Отмет (колено сливное)', tips);
  push('Крепление трубы/саморез', pipeHolders);

 } else if (metalBrand === 'docke_premium_stal'){
  // Döcke Premium Stal
  push('Желоб полукруглый 3 м', gutters);
  push('Соединитель желобов', gutterConnectors);
  push('Втулка соединительная (оцинков.сталь)', gutterConnectors);

  push('Заглушка желоба', caps);

  // углы 90° (135° удалены)
  if ((roofType === 'hip' || roofType === 'tent') && corners > 0){
   push('Угол внешний 90°', corners);
  } else {
   if ((Number(intr) || 0) > 0) push('Угол внутренний 90°', intr);
   if ((Number(ext) || 0) > 0) push('Угол внешний 90°', ext);
  }

  if (roofMountedYes) push('Карнизный крюк короткий', holders);
  else push('Карнизный крюк длинный', holders);
  if (roofMountedYes) push('Кронштейн', holders);

  push('Воронка желоба', funnelsOut);
  push('Воронка водосборная', funnelsWater);
  push('Паук', funnelsTotal);

  push('Труба водосточная 1 м', pipes1);
  push('Труба водосточная 3 м', pipes3);

  push('Муфта соединительная', Math.max(0, (pipes1 + pipes2 + pipes3) - 1));
  push('Колено 72°', elbows);
  push('Отвод трубы', tips);
  push('Хомут трубы', pipeHolders);

 } else if (String(metalBrand).startsWith('gl_optima')){
  // Grand Line Optima (все варианты)
  push('Желоб полукруглый, 3 м', gutters);
  push('Соединитель желоба', gutterConnectors);

  if (roofMountedYes) push('Крюк короткий', holders);
  else push('Крюк длинный', holders);

  push('Заглушка желоба', caps);
  push('Угол желоба 90°', corners);

  push('Воронка', funnelsOut);

  push('Колено трубы 60°', elbows);
  push('Труба круглая, 1 м', pipes1);
  push('Труба круглая, 2 м. NEW* (под заказ)', pipes2);
  push('Труба круглая, 3 м', pipes3);

  push('Колено стока', tips);
  push('Кронштейн трубы на кирпич', pipeHolders);
  push('Кронштейн трубы на дерево', pipeHolders);

 } else {
  // Grand Line Granite (все варианты)
  push('Полукруглый желоб, 3 м', gutters);
  push('Соединитель желоба', gutterConnectors);

  // "полоса" по формуле L/0,6
  if (roofMountedYes) push('Крюк короткий полоса', holders);
  else push('Крюк длинный полоса', holders);
  ...
  push('Колено трубы, 60°', elbows);
  push('Круглая труба соединительная, 1 м', pipes1);
  push('Круглая труба, 3 м', pipes3);
 ...
 // ===== PLASTIC =====
 if (plasticBrand === 'docke_lux'){
  push('Желоб водосточный 3000', gutters);
  push('Соединитель желобов', gutterConnectors);

  if (roofMountedYes){
   push('Кронштейн желоба', holders);
   push('Кронштейн желоба металлический 300мм', holders);
  }

  push('Заглушка желоба', caps);
  push('Угловой элемент желоба 90°', corners);

  push('Воронка выпускная', funnelsOut);
  push('Воронка водосборная', funnelsWater);

  push('Колено', elbows);

  push('Труба водосточная 100х1000', pipes1);
  push('Труба водосточная 3 м', pipes3);
  push('Муфта соединительная', couplings);

  push('Наконечник', tips);
  push('Хомут универсальный', pipeHolders);
  push('Шпилька специальная с гайкой 120мм', pipeHolders);

 } else if (plasticBrand === 'docke_premium'){
  push('Желоб водосточный', gutters);
  push('Соединитель желобов', gutterConnectors);
...
push('Наконечник', tips);
  push('Хомут универсальный', pipeHolders);
  push('Шпилька специальная с гайкой', pipeHolders);

 } else {
  // Döcke Standart: желоб 3 м + добор 1 м
const stBase3 = Math.floor(L / 3); // кол-во желобов 3 м
const stRem = L - stBase3 * 3;
const st1 = (stRem > 0.000001) ? Math.ceil(stRem) : 0; // добор 1 м (1-2 шт)

  const stGutTotal = stBase3 + st1;
  const stConnectors = Math.max(0, stGutTotal - 1);

  push('Желоб водосточный 3 м', stBase3);
  push('Желоб водосточный 1 м', st1);
  push('Соединитель желобов', stConnectors);
 ...
 if (!rows.length){
  gErr.textContent = 'Не удалось рассчитать. Проверьте входные данные.';
  return;
 }
...
// init water calculator
(function gInit(){
 if (!gType || !gCalcBtn) return;

 gRenderType();
 gType.addEventListener('change', gRenderType);

 gMetalBrand?.addEventListener('change', gUpdateValleysVisibility);

 if (gRoofPicks){
  const btns = gRoofPicks.querySelectorAll('.roof-btn');
  btns.forEach(b => b.addEventListener('click', () => gSetActiveRoof(b.dataset.value)));
 }

 gContour?.addEventListener('change', () => {
  if (gOpenWrap) gOpenWrap.classList.toggle('hidden', gContour.value !== 'no');
 });
...
const SOFFIT_WIDTHS_MM = {
  // рабочая ширина для расчёта количества листов
  c8: { work: 1150, full: 1200 },
  evrobrus: { work: 240, full: 264 }
};
...
// ============================================================
 ...
 // --- справочники ширины (мм) ---
  // --- справочники ширины (мм) ---
 // Если в названии профиля есть "TOTAL/WORK", то ширина берётся из него (max/min).
 const WIDTH_WORK_MM = {
  // справочник для профилей без размеров вида "X/Y" в названии
  'Европланка 126': 126,
  'Евротрапеция 118': 118,
 };
 ...
 const WIDTH_TOTAL_MM = {
  ...
  const ROOF_PROFILES = {
  profnastil: [
   'С8 1200/1150','С10 1170/1100','С16 1050/1000','НС18 1150/1100','Н21 1065/1010',
   'НС35 1055/1000','НС44 1050/1000','НС60 930/860','Н75 800/740','Н114 670/620'
  ],
  tile: ['Супермонтеррей 1180/1100','Панорама 1200/1500'],
 };
  ...
  profnastil: ['С8 1200/1150','С10 1170/1100','С16 1050/1000'],
  siding: ['Корабельная доска 260/226','Блок-хаус 356/330','Евробрус 264/240'],
 };
 ...
 function fmtInt(v){
  ...
  // Если в названии есть "число/число" — берём min как рабочую ширину.
  const m = String(key).match(/(\d{2,4})\s*\/\s*(\d{2,4})/);
  if (m){
   const a = Number(m[1]);
   const b = Number(m[2]);
   const mm = Math.min(a,b);
   return mm ? (mm/1000) : 0;
  }
  ...
  // Если в названии есть "число/число" — берём max как общую ширину.
  const m = String(key).match(/(\d{2,4})\s*\/\s*(\d{2,4})/);
  if (m){
   const a = Number(m[1]);
   const b = Number(m[2]);
   const mm = Math.max(a,b);
   return mm ? (mm/1000) : 0;
  }
 ...
 // ============================================================
 // SOFFIT калькулятор — привязка элементов
 // ============================================================
 sMaterial = document.getElementById('s_material');
 sOverhang = document.getElementById('s_overhang');
 sLength = document.getElementById('s_length');
 sTrimP = document.getElementById('s_trim_p');
 sTrimJ = document.getElementById('s_trim_j');
 sTrimFacia = document.getElementById('s_trim_facia');
 sTrimCorner = document.getElementById('s_trim_corner');
 sCalcBtn = document.getElementById('s_calc');
 sPdfBtn = document.getElementById('s_pdf');
 sRes = document.getElementById('s_result');
 sErr = document.getElementById('s_err');
